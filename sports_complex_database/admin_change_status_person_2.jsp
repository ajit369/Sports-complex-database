...
try{
    ...
    int columnCount=metadata.getColumnCount();
     int count=0;
     res=myStmt.executeQuery("SELECT COUNT(PERSON_ID) FROM SPORTS_PERSON WHERE PERSON_ID="+change_id);
     while(res.next())
     {
    	 count=res.getInt(1);
     }
     if(count==0)
     {
    	 ...
    	 out.println("<h1 style='color:red'>There are no sportsmen available with this id</h1>");
    	 ...
    	 out.println("<button type='submit'>SGo back to previous page</button></form> ");
     }
     else{
    	 //out.println(name);
    	 //out.println("<br>"+status);
    	 myStmt.executeUpdate("UPDATE SPORTS_PERSON SET STATUS=+'"+status+"' WHERE PERSON_ID="+change_id);
    	 out.println("<form action='admin_change_details.jsp'>");
