...
//response.setContentType("text/html;charset=UTF-8");
    ...
    //   res=myStmt.executeQuery("SELECT COUNT(username) FROM STUDENT WHERE username='"+name+"'");
      // ResultSetMetaData metadata=res.getMetaData();
       ...
       out.println("<center><h1 style='textDecoration:underline'>Available timings are:</h1><form action='student_add_sport_last.jsp'>");
     //  ResultSet res3=myStmt.executeQuery("SELECT MAX(STUDENT_ID) FROM STUDENT");
       ...
       //}
       ResultSet res312=myStmt.executeQuery("SELECT STUDENT_ID FROM STUDENT WHERE USERNAME='"+name+"'");
       while(res312.next()){
    	   stud_id=res312.getInt(1);
       }
      ResultSet res3=myStmt.executeQuery("SELECT COUNT(*) FROM COACHES WHERE PERSON_ID="+stud_id+" AND STAFF_ID="+id);
      int cnt=0;
      while(res3.next())
      {
   	   cnt=res3.getInt(1);
