...
try{
    ...
    res=myStmt.executeQuery("SELECT COUNT(username) FROM maintenance_staff WHERE username='"+name+"'and password='"+pass+"' AND M_ID<>"+sec_id);
...
//    }
    ...
    myStmt.executeUpdate("UPDATE maintenance_staff SET USERNAME='"+name+"',PASSWORD='"+pass+"' WHERE M_ID="+sec_id);
    out.println("<center><h1 style='color:green; font-family:sans-serif'>Successfully updated your data</h1></center>");
    out.println("<br>");
    //out.println("<form action='maintenan'>");
    //out.println("<input type='text' style='position: fixed; visibility:hidden' value='"+name+"' name='uname' readonly>");
    //out.println("<input type='text' style='position: fixed; visibility:hidden' value='"+pass+"' name='psw' readonly>");
    out.println("<a href='maintenancelog.html'>Go back to home page</a>");
    con.close();
    }}
